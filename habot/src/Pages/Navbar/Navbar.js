...
>
  <a href="#" className="flex items-center md:w-[181px] lg:w-[181px] w-[120px]">
    ...
    {navigation.map((item) => (
      <div key={item.name} className="flex items-center md:text-[14px]">
        ...
        {item.name === 'Find Service Tags' && (
          <img src={dropVec} className="md:justify-center cursor-pointer ml-[7px]" alt="Dropdown" />
        )}
      </div>
    ))}
      ...
      <div className="flex-1 gap-4 flex flex-col md:flex-row">
        {navigation.map((item) => (
          <div key={item.name} className="flex items-center">
            <a href={item.href} aria-current={item.current ? 'page' : undefined} className={classNames(item.current)}>
              {item.name}
            </a>
            {item.name === 'Find Service Tags' && (
              <img src={dropVec} alt="Dropdown" style={{ marginLeft: '7px', cursor: 'pointer' }} />
            )}
          </div>
        ))}
